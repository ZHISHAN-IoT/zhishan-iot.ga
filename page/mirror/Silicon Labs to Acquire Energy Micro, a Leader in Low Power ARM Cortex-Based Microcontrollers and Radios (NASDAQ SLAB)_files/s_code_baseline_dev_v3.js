...
/////////////////////////////////////////////////////////////
//This script will look at the URL, parse out the URL, 
//and then state it as prod and add global 
//reportsuite to the report suite ndq_account. 3.27.2013 v.1.1
//Added Blacklist 4.03.2013 v.1.2
//Added company prefix ngs for 600 rsid 5.13.2013 v.3
/////////////////////////////////////////////////////////////

if (typeof ndq_account !== 'undefined') {
	if (ndq_account.length > 0){
		if (ndq_account.indexOf("nasdaqgcsglobalprod") < 0){
			ndq_account = ndq_account + ',nasdaqgcsglobalprod';
			}
		} else {
			ndq_account = ndqCreateSAccount();		
		}
} else {
	ndq_account = ndqCreateSAccount();
}

function ndqCreateSAccount(){
	
	var omni_prodReportSuiteWithGlobal = "prod" + ",nasdaqgcsglobalprod";    //This is for production primary suite and the production global suite.
	var omni_prodReportSuiteWithDev = "dev" + ",nasdaqgcsglobaldev";;        //This is for dev primary suite and the dev global suite.
	var omni_hostNameStr = window.location.hostname;
	if (!sa) {
	if (omni_hostNameStr == "investor.shareholder.com") { 
	var omni_pathNameCleaned = window.location.pathname.toLowerCase().split('/')[1];
	var sa = "ngs" + omni_pathNameCleaned + omni_prodReportSuiteWithGlobal;
	
	// alert (sa + " this is pathname account");   //Test alert for checking the pathname pull from investor.shareholder.com
	
	}  
	else if (omni_hostNameStr == "investor-staging.shareholder.com") {
	var omni_pathNameCleaned = window.location.pathname.toLowerCase().split('/')[1];
	var sa = "nasdaqgcs" + omni_pathNameCleaned + omni_prodReportSuiteWithDev;  //This is for manually created report suite ID;
	
	// alert (sa + " this is pathname account");   //Test alert for checking the pathname pull from investor-staging.shareholder.com
	
	} 
	else 
	  {
	    var omni_hostNameStrReplacedWWW = omni_hostNameStr.replace(/\.client\.shareholder/gi,"").replace(/investor\./gi,"").replace(/investors\./gi,"").replace(/www\./gi,"").replace(/\.com/gi,"").replace(/\./gi,"-");
	    var sa                   = "ngs" + omni_hostNameStrReplacedWWW + omni_prodReportSuiteWithGlobal;  
	    
	
	// alert (sa + " this is hostname account");   //Test alert for checking the hostname pull from non-investor.shareholder sites
	 }
	 }
	return sa;
}

var s=s_gi(ndq_account)
...
s.linkInternalFilters="javascript:,shareholder.com/visitors," + window.location.host;  //This will set the current location as the internal site.
...
function s_doPlugins(s) {
    ...
    s.eVar7="D=g";
    s.eVar8=D=window.location.hostname;
	
	/* Plugin: getQueryParam 2.3 */
	s.campaign=s.getQueryParam('cmpid'); //replace 'cmpid' with correct campaign query parameter
	
	/* Plugin: getNewRepeat 1.0 */
	s.eVar11=s.getNewRepeat();

    /* Plugin: getVisitNum 2.0 */
	s.eVar12=s.getVisitNum();

    /* Plugin: getPercentPageView v1.2 
	s.prop13=s.getPercentPageViewed(); */

    /* Plugin: getPreviousValue_v1.0 */
    s.prop14=s.getPreviousValue(s.pageName,'gpv_p14','');
...
var s=s_gi(ndq_account);if(event.target){obj=event.target
...
if (!(exitDomain == "investor-staging.shareholder.com" || exitDomain == "apps2.shareholder.com" || exitDomain == "files.shareholder.com" || exitDomain == "apps.shareholder.com")) {
...
s.visitorNamespace="nasdaqgcs"
s.trackingServer="nasdaqgcs.d2.sc.omtrdc.net"
